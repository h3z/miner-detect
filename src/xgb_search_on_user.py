...
def exp(features, df, params):
    ...
    if 'ELEC_TYPE_NAME' in features:
        x = pd.concat([x, pd.get_dummies(x.ELEC_TYPE_NAME)], axis=1).drop(columns=['ELEC_TYPE_NAME'])
        x_val = pd.concat([x_val, pd.get_dummies(x_val.ELEC_TYPE_NAME)], axis=1).drop(columns=['ELEC_TYPE_NAME'])

    if 'VOLT_NAME' in features:
        x = pd.concat([x, pd.get_dummies(x.VOLT_NAME)], axis=1).drop(columns=['VOLT_NAME'])
        x_val = pd.concat([x_val, pd.get_dummies(x_val.VOLT_NAME)], axis=1).drop(columns=['VOLT_NAME'])
...
def today_exp(params):
    cols = hol_diff_col + hol_mean_col + hol_std_col + day_col + user_col + month_col
    t = ds.train

    pred_val = exp(cols, t, params)

    ids = set(pred_val.query('pred == 1').id.unique())
    return [0, len(ids & C.true_ids), len(ids & C.false_ids), len(ids - C.true_ids - C.false_ids), len(ids), str(ids)]
